...
# ── 非数字的编造 ────────────────────────────────────────────────────────
#
# unsupported_numbers 只拦日期、金额、年份。拦不住的是这一类：
#
#     据《新宁县志》记载……      书名号里的东西是凭空来的
#     当地的李建国老师告诉我们   人名是凭空来的
#     始建于明代的金城书院       朝代是凭空来的
#
# 这类比编错数字危险得多：数字有时会跟项目卡里的字段对不上、用户能察觉，
# 而这些读起来跟真的一模一样，跟任何字段都不冲突，用户没有任何线索。
# 而且它一旦被抄进真发出去的推送里，是要署学生自己名字的。
#
# 判据跟数字那套完全一致：只报**在原文和项目卡里都找不到**的。原文里写了
# 的，模型引用它是好事，不能报——警告一多就没人看了，这条在数字那边已经
# 吃过一次亏。

# 书名号最干净：通用词不会用书名号，里面基本都是专名。
_WORK_TITLE_RE = re.compile(r"《([^《》]{1,30})》")

# 人名靠**姓氏**判断，不靠排除词表。
#
# 「当地老师」「带队老师」「一位大爷」跟「李建国老师」「王大爷」的区别，
# 用"排除哪些前缀"去写是补不完的（这个项目上手工清单已经漂过五次）。
# 换成正判据：前缀第一个字必须是常见姓氏。姓氏是封闭集合，不会漂。
_SURNAMES = frozenset(
    ...
    "胡凌霍虞万支柯昝管卢莫房裘缪干解应宗丁宣贲邓郁单杭洪包诸左石崔"
    "吉钮龚程嵇邢滑裴陆荣翁荀羊於惠甄曲家封芮羿储靳汲邴糜松井段富巫"
)
# 只抓**虚构引语**里的人名，不抓所有人名。
#
# 抓所有人名是做不到的：中文没有空格，正则只能贪婪切，「任课老师」的
# 「任」、「高三年级主任」的「高」都正好是姓氏，实测全过。要切对得靠 NER，
# 而这个项目是零第三方依赖。
#
# 但编造人名真正的危害集中在一处：虚构的一手材料。
#
#     李建国老师告诉我们，学生基础薄弱     <- 这句话会被原样写进调研报告
#     访谈对象包括任课老师、带队老师        <- 这是泛指，没有危害
#
# 区别在于后面跟不跟说话动词。把判据收到这里，既对准了危害，也顺手把
# 泛指挡在了外面——泛指后面跟的是顿号或「的」，不会跟「告诉」。
_QUOTED_PERSON_RE = re.compile(
    r"([一-鿿]{1,3}(?:教授|副教授|老师|书记|主任|馆长|院长|校长|村长|镇长|县长|"
    r"站长|队长|研究员|工程师|医生|大爷|大娘|阿姨|师傅))"
    r"\s*(?:说|讲|称|谈到|告诉|表示|介绍说|回忆|认为|提到|反映|坦言|强调|指出)")

# 姓氏对了也可能不是人名：「高三年级主任」的「高」正好是姓。
# 人名里不会出现数字，也不会带这些组织/职务字。
_NOT_IN_A_NAME = frozenset("一二三四五六七八九十百千万0123456789"
                           "年级班系院部处办组队室科站所中心")

# 机构名这条判据砍掉了，原因记在这里，免得以后有人再加一遍。
#
# 试过 `[一-鿿]{2,10}(?:大学|中学|村委会|合作社|基金会|…)`，拿 5 个真实项目
# 生成的推送和调研报告一量，10 条告警 10 条误报：
#
#     「与高中生聊聊大学」「本报告基于清华大学」「他们与村委会」
#     「短期支教对县域高中学」「本项目与素兰基金会」（素兰基金会原文里有）
#
# 两个原因叠在一起，都不好解：
#   1. 中文没有空格，正则只能贪婪地往左吃 2-10 个字，切出来的不是名词短语。
#      要切对得靠分词，而这个项目是零第三方依赖，手写中文分词是无底洞。
#   2. 更根本的是，在"大学生去中学支教"这个语境里，「大学」「中学」
#      「村委会」「合作社」本来就是高频普通词，它们不是专名信号。
#
# 少而准好过多而吵——告警一多就没人看，这条在数字那边已经吃过一次亏。
# 书名号和年代是硬边界或封闭枚举，留下；人名靠姓氏正判据，留下。

# 朝代、年代。枚举，不用「明」「清」单字——那会命中"明确""清华"。
_ERA_RE = re.compile(
    r"((?:秦|汉|唐|宋|元|明|清)(?:代|朝)|民国(?:时期|年间)?|建国初期?|"
    r"上世纪\d{2}年代|\d{2}世纪)")


def _proper_nouns(text: str) -> dict[str, set[str]]:
    """按类别挑出文本里的专名。"""
    people = {
        match.group(1)
        for match in _QUOTED_PERSON_RE.finditer(text)
        if match.group(1)[0] in _SURNAMES
        and not (set(match.group(1)) & _NOT_IN_A_NAME)
    }
    return {
        "作品或文献": {m.group(1) for m in _WORK_TITLE_RE.finditer(text)},
        "引述的人": people,
        "年代": {m.group(1) for m in _ERA_RE.finditer(text)},
    }


def unsupported_names(draft: str, facts: str) -> list[str]:
    """生成稿里出现、但在原文和项目卡里都查不到的专名。

    返回的是「类别：内容」，因为这几类的严重程度不一样——编一个人名和
    写错一个朝代，读者需要核实的方式完全不同。
    """
    known = _proper_nouns(facts)
    found = _proper_nouns(draft)
    out: list[str] = []
    for kind in ("作品或文献", "引述的人", "年代"):
        # 整串在材料里出现过就算有出处——比如原文写了「新宁县第一中学」，
        # 文案再提一次不该报。
        missing = sorted(item for item in found[kind] - known[kind] if item not in facts)
        out += [f"{kind}「{item}」" for item in missing]
    return out


def fabrication_warning(draft: str, facts: str, *, where: str) -> str:
    """生成稿里查不到出处的东西，统一列在这里。

    三个出口（写推送、写调研报告、改上一版）共用这一段措辞。原来是三处
    各写一份字符串，改了一处会忘另外两处——这个项目上并列的手工清单已经
    漂过五次，能合就合。
    """
    numbers = unsupported_numbers(draft, facts)
    names = unsupported_names(draft, facts)
    if not numbers and not names:
        return ""
    rows = [f"\n\n> **下面这些在{where}里查不到出处**，用之前先自己核实一遍："]
    if numbers:
        rows.append("> - 数字：" + "、".join(numbers))
    if names:
        rows.append("> - 专名：" + "、".join(names)
                    + "。人名、书名、机构名一旦写错，是要署你自己名字发出去的。")
    return "\n".join(rows)
...
class PracticeChatAdapter:
    ...
    _REPORT_SYSTEM_PROMPT = (
        ...
        "## 附录，最后给一张证据台账**空表**——只要表头和列名，**不要填示例行**。\n"
        "（实测模型会在示例行里填出「《某某县乡村旅游发展情况介绍》宣传册」这种"
        "看起来完全像真的文件名，学生会当成需要去找的具体材料。）\n"
        "总长 800-1200 字。"
    )
    ...
    def _draft_report(self, project: dict[str, Any]) -> ChatResult:
        ...
        tail = fabrication_warning(body, f"{facts}\n{source}", where="原文和项目卡")
        tail += ("\n\n---\n> 这份框架依据的是该项目的招募通知原文。"
    ...
    def _draft_post(self, messages: list[dict[str, Any]], all_user_text: str, latest: str) -> ChatResult:
        ...
        def produce() -> Iterable[str]:
            ...
            try:
                ...
                return
            # 数字和专名是最容易被顺手编出来、也最容易被当真的东西。逐个回查，
