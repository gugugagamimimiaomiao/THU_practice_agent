...
> 2026-08-16 更新：公众平台历史列表接口持续返回 `200013`。当前默认使用 WeWe RSS 的微信读书通道，且只增量扫描清华大学学生会、清华大学学生社团、清华紫荆之声、清华大学学生公益和清华大学乡村振兴工作站最近 28 天的文章。扫码后新激活公众号会额外进行一页索引同步和最长一年的受控历史机会回填；`scripts/wewe_collector.py` 按服务器约定的标题规则预筛，并为短正文保留图片 URL；`we-mp-rss` 仅保留为诊断备用。持久化、令牌失效处理、扫码后的自动收尾、服务器投递与每日更新见 [WEWE_DEPLOYMENT.md](WEWE_DEPLOYMENT.md)。
