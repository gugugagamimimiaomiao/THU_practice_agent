...
KNOWN_DEPARTMENTS = [
    ...
    "求真书院", "笃实书院",
    # 这一批是 2026-08-29 被指出漏掉的。依据：订阅表 wechat_sources 里有它们的
    # 官方号（清华大学无穹书院 / 水木书院 / 自强书院 / 紫荆书院、THU臻于至善、
    # THU为先），且库里的文章正文里出现过「为先书院」「至善书院」「水木书院」
    # 「苏世民书院」的完整写法。
    "为先书院", "至善书院", "水木书院", "自强书院", "紫荆书院", "无穹书院",
    "苏世民书院",
]
...
# **这几个书院故意没有简称**，别看着"缺了"就补上去。
#
#   紫荆  实测在 73 条真实项目里撞 3 次（志愿中心组长招募、学生骨干招募、
#         「再续紫荆情」），还撞上校级号「清华紫荆之声」。紫荆是校花、
#         是宿舍区、是一堆组织的名字。
#   水木  「水木清华」。土木系的号就叫「水木华声」。
#   自强  校训「自强不息」。
#   至善  「臻于至善」，一句到处都在用的口号。
#   为先  「以…为先」是普通搭配。
#
# 水木/自强/至善/为先 在当前 73 条里撞 0 次——但那只说明**这批数据里没出现**，
# 不说明安全。判据是这个词在清华语境里是不是专名，不是当前样本的计数；
# 样本一大它们必撞。这几个书院靠公众号别名和全称匹配，够用了。
_DELIBERATELY_NO_SHORT_NAME = ("紫荆书院", "水木书院", "自强书院", "至善书院",
                               "为先书院", "苏世民书院")
...
DEPARTMENT_ACCOUNT_ALIASES = {
    ...
    "公管声音": "公管学院",
    "清美团宣": "美术学院",
    # —— 书院号里号名不含全称、简称又不能给的那几个 ——
    # （清华大学无穹/水木/自强/紫荆书院这几个号名里带全称，走全称匹配即可）
    "THU臻于至善": "至善书院",
    "THU为先": "为先书院",
    "自强eEnery": "自强书院",
}

# 订阅表里这几个号我查不出对应哪个书院，**故意留空**：
#   雅人新致      像是新雅的文字游戏，但也可能是别的，没有正文佐证
#   THU水木秀钟   「秀钟」出处不明
#   THU水清木华   「水清木华」是清华园的旧称，未必指水木书院
# 猜错了没人看得出来，而后果是把项目静默归给错误的书院。等采到它们的文章、
# 正文里能对上了再补。
